 ...
 * about efficiency made in {@link com.uwyn.jhighlight.fastutil.objects.ObjectArrays} apply here.
 *
...
public class ObjectArrayList <K> extends AbstractObjectList <K> implements RandomAccess, Cloneable, java.io.Serializable {
 ...
 private void grow( final int capacity ) {
  if ( wrapped ) a = ObjectArrays.grow( a, capacity, size );
   ...
   if ( capacity > a.length ) {
    final int newLength = (int)Math.max( Math.min( 2L * a.length, com.uwyn.jhighlight.fastutil.Arrays.MAX_ARRAY_SIZE ), capacity );
 ...
 public void removeElements( final int from, final int to ) {
  com.uwyn.jhighlight.fastutil.Arrays.ensureFromTo( size, from, to );
