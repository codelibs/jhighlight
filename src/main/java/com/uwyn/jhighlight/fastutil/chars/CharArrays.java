 ...
 *
 * <P>Note that {@link com.uwyn.jhighlight.fastutil.io.BinIO} and {@link com.uwyn.jhighlight.fastutil.io.TextIO}
 ...
 public static void radixSortIndirect( final int[] perm, final char[] a, final int from, final int to, final boolean stable ) {
  ...
  while( offsetPos > 0 ) {
   ...
   if ( stable ) {
    ...
    com.uwyn.jhighlight.fastutil.ints.IntArrays.fill( count, 0 );
   }
   else {
    // When all slots are OK, the last slot is necessarily OK.
    final int end = length - count[ lastUsed ];
